...
| [ADR-012 Previous Session Relative Range frontend integration](ADR-012-previous-session-relative-range-frontend-integration.md) | ADR-012 | FE-PSRR1-31: thin frontend for `previous_session_relative_range` (presentation object + page + `/scanners/previous-session-relative-range` route + nav "Previous Session Relative Range" / column "Relative Range") reusing the generic scanner UI unchanged; **one new** display-only `formatRatioMultiple` renders the dimensionless `relative_range_ratio` as `0.42×` (ratio, not %, so `formatPercent` N/A); backend rank ASCENDING authoritative (rank 1 = most compressed vs own 20-session baseline), no client calc/re-rank; non-directional (compression, not a breakout signal); first multi-session frontend (PARTIAL more likely, handled identically); design-system baseline = React+TS+Tailwind + Material 3 principles + Material Symbols + StepOne dark, no Bootstrap/MUI; Narrow CPR/PSR/PSB frozen; no backend/PG/WS/authority change; implementation deferred |
| [ADR-016 sector metrics definitions](ADR-016-sector-metrics-definitions.md) | ADR-016 | SECTOR-3 pure metrics math: Decimal ratio returns; overnight/intraday/total decomposition (intraday drives direction); equal-weight breadth (no cap weighting); median central tendency; MAD + IQR (Tukey exclusive hinges, `None` for N<2); directional agreement/participation; raw direction requiring median-tilt AND breadth-tilt agreement (else MIXED); F&O-universe median proxy (not NIFTY) + raw relative strength; coverage/freshness fail-closed; all thresholds UN-CALIBRATED policy inputs (SECTOR-5 calibrates); no score/confidence/ranking |
| [ADR-016 stock participation & ranking definitions](ADR-016-sector-stock-participation-definitions.md) | ADR-016 | SECTOR-4 pure stock layer: three independent facts (absolute, stock_vs_sector, stock_vs_universe); sector alignment (ALIGNED/OPPOSED/NEUTRAL/MIXED_CONTEXT/INSUFFICIENT_DATA); sector-signed directional strength; within-sector competition rank + percentile `(N−rank)/(N−1)` (None for N<2), identity-only display tiebreak; directional ranking only when sector BULLISH/BEARISH; diagnostic robust magnitude `(x−median)/MAD` (None on MAD 0); freshness/stale exclusion + duplicate/sector/date fail-closed; bull/bear-symmetric, order/scale-invariant; RAW/UN-CALIBRATED — no StockParticipationScore, no LEADER/PARTICIPANT/LAGGARD/DIVERGENT threshold, no confidence (SECTOR-5) |
